        ...
        Auth.findOne({ authToken: req.header('authToken') || req.params.authToken || req.body.authToken || req.query.authToken }, (err, authDetails) => {
            if (err) {
                ...
                token.verifyToken(authDetails.authToken, authDetails.tokenSecret, (err, decoded) => {
                    ...
                    else if (decoded.data.permissions == 'admin' || decoded.data.permissions == 'Admin') {
                        req.user = { userId: decoded.data.userId }
                        next()
                    }
                    else {
                        logger.error('Not authorized', 'Authorization Middleware:Admin', 7)
                        let apiResponse = responseLib.generate(true, 'You dont have permissions to access this route', 401, null)
                        res.send(apiResponse)
                    }
                });// end verify token
